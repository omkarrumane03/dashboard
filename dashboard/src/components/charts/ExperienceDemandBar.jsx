...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#0d1117', border: `1px solid ${PALETTE.border}`, borderRadius: 8, padding: '8px 12px', fontFamily: "Inter, sans-serif", fontSize: 16 }}>
      ...
      {BUCKETS.map(bucket => {
        ...
        return (
          <div key={bucket} style={{ marginBottom: 6 }}>
            <div style={{ color: BUCKET_COLORS[bucket], fontWeight: 'bold', fontSize: 18, marginBottom: 2 }}>
              {BUCKET_LABELS[bucket]}
            </div>
            {grouped[bucket].map((role, idx) => (
              <div key={idx} style={{ color: '#ffffff', paddingLeft: 6, fontSize: 18, lineHeight: '1.4' }}>
                • {role.shortTitle}
