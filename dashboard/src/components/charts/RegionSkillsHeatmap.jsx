...
const CustomXAxisTick = ({ x, y, payload, uniqueRoles }) => {
  ...
  const truncated = truncateLabel(fullLabel, 12);
  return (
...
const CustomYAxisTick = ({ x, y, payload, uniqueLocations }) => {
  ...
  const truncated = truncateLabel(fullLabel, 15);
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={-10} y={4} textAnchor="end"
        fill={PALETTE.muted} fontSize={11}
        fontFamily="'JetBrains Mono', monospace" style={{ cursor: 'pointer' }}>
        <title>{fullLabel}</title>
        {truncated}
      </text>
    </g>
...
export default function RolesLocation() {
  // All 6 months shown; Jan/Feb/Apr will render an empty heatmap
  const [selectedPeriod, setSelectedPeriod] = useState('May');
  const periods = ['Dec', 'Jan', 'Feb', 'Mar', 'Apr', 'May'];

  const filteredData    = orionPipeline.filter(item => item.month === selectedPeriod);
  ...
  uniqueRoles.forEach((role, xIdx) => {
    uniqueLocations.forEach((loc, yIdx) => {
      const matches = filteredData.filter(
        item => item.jobTitle === role && (item.location || 'Remote/Global') === loc
      );
      const totalOpenings = matches.reduce((sum, item) => sum + (parseInt(item.openings, 10) || 0), 0);
      if (totalOpenings > 0) {
        matrixData.push({ x: xIdx, y: yIdx, roleName: role, locationName: loc, openings: totalOpenings });
      }
    });
  });
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, paddingBottom: 4, flexWrap: 'wrap' }}>
        {periods.map((period) => (
          <button key={period} onClick={() => setSelectedPeriod(period)} style={{
            ...
            cursor: 'pointer', fontWeight: selectedPeriod === period ? 600 : 400,
      ...
      {matrixData.length === 0 ? (
        <div style={{
          ...
          color: PALETTE.muted, fontFamily: "'JetBrains Mono', monospace", fontSize: 13,
          border: `1px dashed ${PALETTE.border}`, borderRadius: 8,
        }}>
          No roles opened in {selectedPeriod}
        </div>
      ) : (
