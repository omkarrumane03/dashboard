  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={salaryTrend} margin={{ top: 20, right: 10, left: -10, bottom: 5 }}>
        ...
        <XAxis dataKey="domain" tick={{ fill: PALETTE.muted, fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} axisLine={{ stroke: PALETTE.border }} tickLine={false} />
        <YAxis domain={[60, 130]} tick={{ fill: PALETTE.muted, fontSize: 10, fontFamily: "'JetBrains Mono', monospace" }} axisLine={false} tickLine={false} unit="K" />
        <Tooltip content={<CustomTooltip />} />
        <Bar dataKey="avgSalary" name="Avg Salary ($K)" radius={[4, 4, 0, 0]}>
          <LabelList dataKey="avgSalary" position="top" style={{ fill: PALETTE.muted, fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} formatter={v => `$${v}K`} />
